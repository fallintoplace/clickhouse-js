...
class Parser {
  ...
  run(): ParseResult {
    ...
    for (const tok of this.tokens)
      if (tok.type === TokenType.Error) return Parser.fail(tok.begin, tok.text);

    const node = this.parseType();
    if (!node) {
      if (this.hard_error) return makeResult(null, this.hard_error);
    ...
    if (this.cur().type !== TokenType.End)
      return Parser.fail(
        this.cur().begin,
        "unexpected trailing input after the data type",
      );
  ...
  private advance(): void {
    if ((this.tokens[this.pos] as Token).type !== TokenType.End) ++this.pos;
  }
  ...
  private setHardError(at: number, msg: string): void {
    if (!this.hard_error) this.hard_error = { message: msg, position: at };
  }

  private isIdentifier(): boolean {
    return (
      this.type() === TokenType.Word || this.type() === TokenType.QuotedIdent
    );
  }
  ...
  private parseIdentifier(): { ok: boolean; name: string } {
    if (!this.isIdentifier()) return { ok: false, name: "" };
  ...
  private parseType(): Node | null {
    ...
    if (suffix !== "") type_name = type_name_upper + " " + suffix;

    this.skipTrailingComma();

    /// Enum special case -> EnumDataType with explicit values.
    if (
      isEnumTypeUpper(type_name_upper) &&
    ...
    if (this.type() !== TokenType.OpeningParen) return node;
    this.advance();

    if (!this.parseArgumentList(type_name, node.arguments)) return null;

    if (this.type() !== TokenType.ClosingParen) return null;
    this.advance();
  ...
  private parseTypeNameSuffix(u: string): string {
    if (u === "NATIONAL") {
      ...
      if (this.type() === TokenType.OpeningParen) {
        const saved = this.pos;
        this.advance();
        if (this.type() === TokenType.Number) this.advance();
  ...
  private skipTrailingComma(): void {
    if (
      ...
      (this.tokens[this.pos + 1] as Token).type === TokenType.ClosingParen
    )
      this.advance();
  }
  ...
  private parseEnumValues(values: EnumValue[]): boolean {
    ...
    while (true) {
      if (!first) {
        if (this.type() !== TokenType.Comma) break;
        this.advance();
      }
      first = false;

      if (this.type() !== TokenType.String) return false;
      const name = this.cur().text;
      this.advance();

      if (this.type() !== TokenType.Equals) return false;
      this.advance();
      ...
      if (this.type() !== TokenType.Number || this.cur().is_float) return false;
  ...
  private parseTuple(type_name: string): Node | null {
    ...
    while (true) {
      if (!first) {
        if (this.type() === TokenType.Comma) this.advance();
        else break;
      }
      first = false;
    ...
    if (this.type() === TokenType.ClosingParen && node.arguments.length !== 0) {
      this.advance();
      node.has_argument_list = true;
      if (has_named) node.element_names = names;
      return node;
    }
  ...
  private parseArgumentList(type_name: string, out: Node[]): boolean {
    ...
    if (
      ...
      this.setHardError(
        this.cur().begin,
        type_name + " is not supported by this parser yet",
      );
      return false;
    }
    if (lower === "json") {
      this.setHardError(
        this.cur().begin,
        "JSON typed/object arguments are not supported by this parser yet",
      );
      return false;
    }
    ...
    while (true) {
      ...
      let arg: Node | null;
      if (type_name === "Dynamic") arg = this.parseEqualsArgument();
      else if (type_name === "Nested") arg = this.parseNameTypePair();
      else if (type_name === "Tuple") arg = this.parseNameTypePairOrType();
  ...
  private parseEqualsArgument(): Node | null {
    const id = this.parseIdentifier();
    if (!id.ok) return null;
    if (this.type() !== TokenType.Equals) return null;
    this.advance();
